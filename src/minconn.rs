      ...
      METIS_OBJTYPE_CUT =>
      {
        ...
        for ii in (pptr[pid as usize])..(pptr[(pid+1) as usize]) {
          ...
          if rinfo[i as usize].ed > 0 {
            ...
            let nbrs  = std::slice::from_raw_parts(ctrl.cnbrpool.add(rinfo[i as usize].inbr as usize), nnbrs);
      ...
      METIS_OBJTYPE_VOL =>
      {
        ...
        for ii in (pptr[pid as usize])..(pptr[(pid+1) as usize]) {
          ...
          if rinfo[i as usize].ned > 0 {
            let nnbrs = rinfo[i as usize].nnbrs as usize;
            let nbrs  = std::slice::from_raw_parts(ctrl.vnbrpool.add(rinfo[i as usize].inbr as usize), nnbrs);
    ...
    /* See if you have enough memory to store the adjacent info for that subdomain */
    {
      ...
      if maxnads[pid as usize] < nads {
...

        {
          ...
          let old_adids = NonNull::slice_from_raw_parts(NonNull::new(adids[pid]).unwrap(), nads as usize);
          adids[pid as usize]   = gk::irealloc(old_adids, maxnads[pid] as usize, 
            c"ComputeSubDomainGraph: adids[pid]").as_buf_ptr();
        }

        {
          mkslice_mut!(ctrl->adwgts, nparts);
          let old_adwgts = NonNull::slice_from_raw_parts(NonNull::new(adwgts[pid]).unwrap(), nads as usize);
          adwgts[pid as usize]  = gk::irealloc(old_adwgts, maxnads[pid] as usize, 
            c"ComputeSubDomainGraph: adids[pid]").as_buf_ptr();
        }
      }
...
pub extern "C" fn EliminateSubDomainEdges(ctrl: *mut ctrl_t, graph: *mut graph_t) {
  ...
  loop {
    ...
    while let Some(me) = queue.pop() {
        ...
        for min in (0)..(cand2.len()) {
          ...
          ifset!(ctrl.dbglvl, METIS_DBG_CONNINFO, 
            print!("\tMinOut: {:4}, to: {:3}, TtlWgt: {:5}[#:{:}]\n", 
              mypmat[other as usize], other, cpwgt.iter().sum::<idx_t>(), nind));
          ...
          for i in (0)..(cand.len()) {
            ...
            if mypmat[k as usize] > 0 {
              ...
              if mcutil::ivecaxpylez(ncon as idx_t, 1, cpwgt.as_ptr(), pwgts[cntrng!(k*ncon, ncon)].as_ptr(), maxpwgt[cntrng!(k*ncon, ncon)].as_ptr()) == 0 {
                continue;
              }

              /* get a dense vector out of k's connectivity */
              for j in (0)..(nads[k as usize])  {
                kpmat[*adids[k as usize].add(j as usize) as usize] = *adwgts[k as usize].add(j as usize);
              }
              ...
              //   }
              // }
              let bad_effects = (0..nparts).any(|j| otherpmat[j] > 0 && kpmat[j] == 0 && nads[j] + 1 >= nads[me]);
              ...
              if !bad_effects { 
                ...
                if nads[k as usize]+nadd < nads[me] {
                  if target2 == usize::MAX || nads[target2 as usize]+bestnadd > nads[k as usize]+nadd ||
      ...
      if target != usize::MAX {
        match ctrl.objtype {
          METIS_OBJTYPE_CUT => MoveGroupMinConnForCut(ctrl, graph, target as idx_t, nind as idx_t, ind.as_mut_ptr()),
          METIS_OBJTYPE_VOL => MoveGroupMinConnForVol(ctrl, graph, target as idx_t, nind as idx_t, ind.as_mut_ptr(),
            vmarker.as_nullable_ptr_mut(), pmarker.as_nullable_ptr_mut(), modind.as_nullable_ptr_mut()),
...
pub extern "C" fn MoveGroupMinConnForCut(ctrl: *mut ctrl_t, graph: *mut graph_t, to: idx_t, nind: idx_t, ind: *mut idx_t) 
{
  ...
  for nind in (0..nind).rev() {
    ...
    let mynbrs = std::slice::from_raw_parts_mut(ctrl.cnbrpool.add(myrinfo.inbr as usize), myrinfo.nnbrs as usize);
    ...
    /* Update pwgts */
    {
      ...
      blas::iaxpy(ncon,  1, &vwgt[cntrng!(i*ncon, ncon)], 1, &mut pwgts[cntrng!(to as usize * ncon, ncon)],   1);
    ...
    UpdateEdgeSubDomainGraph(ctrl, from as idx_t, to, myrinfo.id-mynbrs[k as usize].ed, std::ptr::null_mut());

    /* Update ID/ED and BND related information for the moved vertex */
    UpdateMovedVertexInfoAndBND!(i, from as idx_t, k, to, myrinfo, mynbrs, where_, nbnd, 
    ...
    for j in (xadj[i as usize])..(xadj[(i+1) as usize]) {
      ...
      UpdateAdjacentVertexInfoAndBND!(ctrl, ii, xadj[(ii+1) as usize]-xadj[ii as usize], me,
        from as idx_t, to, myrinfo, adjwgt[j as usize], nbnd, bndptr, bndind, BNDTYPE_REFINE);

      /* Update subdomain graph to reflect the move of 'i' for domains other 
      than 'from' and 'to' */
      if me as usize != from && me != to {
        UpdateEdgeSubDomainGraph(ctrl, from as idx_t, me, -adjwgt[j as usize], std::ptr::null_mut());
...
pub extern "C" fn MoveGroupMinConnForVol(ctrl: *mut ctrl_t, graph: *mut graph_t, to: idx_t, nind: idx_t, ind: *mut idx_t, vmarker: *mut idx_t, pmarker: *mut idx_t, modind: *mut idx_t) 
{
  ...
  for nind in (0..nind).rev() {
    ...
    let mut xgain = if (*myrinfo).nid == 0 && (*myrinfo).ned > 0 { vsize[i as usize] } else { 0 };
    ...
    if let Some(k) = mynbrs.iter().position(|nbr| nbr.pid == to) {
      ...
      for j in (xadj[i as usize])..(xadj[(i+1) as usize]) {
        ...
        let onbrs  = std::slice::from_raw_parts(ctrl.vnbrpool.add(orinfo.inbr as usize), orinfo.nnbrs as usize);
